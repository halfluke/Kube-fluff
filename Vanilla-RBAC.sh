...
# Env: K=/path/to/kubectl  DEBUG_CHECK20=1 DEBUG_CHECK21=1 (debug flags no-op on this script)
# ------------------------------------------------------------
CHECKS_SPEC=""
LIST_CHECKS=0
QUIET=0
...
Usage:
  Vanilla-RBAC.sh [--checks LIST] [--critical] [--quiet] [--debug-check20] [--debug-check21]
...
# Critical set (check 14 configmap audit excluded — noisy / environmental)
...
echo "===== Kubernetes RBAC Security Audit (Critical Only) ====="
...
get_subjects_for_role() {
    ...
    if [[ "$ns" == "cluster" ]]; then
        ...
        crb_subjects=$($K get clusterrolebinding -o json 2>/dev/null \
          | jq -r --arg ROLE "$name" '
              .items[]
        ...
        rb_subjects=$($K get rolebinding -A -o json 2>/dev/null \
          | jq -r --arg ROLE "$name" '
              .items[]
        ...
        subjects=$($K get rolebinding -n "$ns" -o json \
          | jq -r --arg ROLE "$name" '
              .items[]
              | select(.roleRef.name == $ROLE)
              | . as $binding
...
check_permission() {
    ...
    matches=$($K get clusterrole,role -A -o json \
...
for TARGET_NS in "${SYSTEM_CONFIGMAP_NAMESPACES[@]}"; do
  ...
  $K get rolebinding -n "$TARGET_NS" -o json \
  | jq -rc '
      .items[] | . as $rb
      | .roleRef as $ref
  ...
  | while IFS= read -r json; do
      ...
      if [[ "$refKind" == "Role" ]]; then
        rules=$($K get role "$refName" -n "$TARGET_NS" -o json 2>/dev/null || echo "")
      else
        rules=$($K get clusterrole "$refName" -o json 2>/dev/null || echo "")
      fi
      [[ -z "$rules" ]] && continue
  ...
  | while IFS= read -r json; do
      ...
      rules=$($K get clusterrole "$ref" -o json 2>/dev/null || echo "")
      [[ -z "$rules" ]] && continue
