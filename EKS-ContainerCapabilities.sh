...
# Informational: EKS-managed nodes often carry eks.amazonaws.com/nodegroup (or eksctl alpha label).
if NODES_JSON=$(kubectl get nodes -o json 2>/dev/null); then
    if echo "$NODES_JSON" | jq -e '
        [.items[]?.metadata.labels // {}]
        | any(
            (.["eks.amazonaws.com/nodegroup"] // "") != ""
            or (.["alpha.eksctl.io/nodegroup-name"] // "") != ""
          )
    ' >/dev/null 2>&1; then
        echo "[INFO] EKS-style node labels detected (eks.amazonaws.com/nodegroup or alpha.eksctl.io/nodegroup-name): cluster appears EKS-managed."
    fi
fi
...
while read -r NS; do
    ...
    IS_SYSTEM_NS=0
    if [[ "$NS" == openshift-* ]] || [[ "$NS" == kube-* ]] || [[ "$NS" == "aws-node" ]]; then
        IS_SYSTEM_NS=1
    fi
    ...
    while read -r POD; do
        ...
        IRSA_ARN=$(echo "$POD_JSON" | jq -r '.metadata.annotations["eks.amazonaws.com/role-arn"] // empty')
        if [[ -z "$IRSA_ARN" ]]; then
            _sa_json=""
            _sa_json=$(kubectl get serviceaccount "$SA_NAME" -n "$NS" -o json 2>/dev/null) || _sa_json=""
            if [[ -n "$_sa_json" ]]; then
                IRSA_ARN=$(echo "$_sa_json" | jq -r '.metadata.annotations["eks.amazonaws.com/role-arn"] // empty')
            fi
        fi
        [[ -z "$IRSA_ARN" ]] && IRSA_ARN="none"
        ...
        if [[ "$OUTPUT_MODE" == "text" ]]; then
            ...
            echo "\"${NS//\"/\"\"}\",\"${POD//\"/\"\"}\",\"${SA_NAME//\"/\"\"}\",\"$PRIV_DISPLAY\",\"${STATUS//\"/\"\"}\",\"${REQUESTED_CAPS_STR//\"/\"\"}\",\"${DROPPED_CAPS_STR//\"/\"\"}\",\"${EFFECTIVE_ESC}\",\"$ESC_DISPLAY\",\"$HOST_PID\",\"$HOST_NET\",\"$HOST_IPC\",\"${RUN_AS_NONROOT_DISPLAY//\"/\"\"}\",\"${RUN_AS_USER//\"/\"\"}\",\"${RUN_AS_GROUP_DISPLAY//\"/\"\"}\",\"$FSGROUP\",\"${SUPP_GROUPS//\"/\"\"}\",\"${VOLUME_TYPES_DISPLAY//\"/\"\"}\",\"${AUTOMOUNT_DISPLAY//\"/\"\"}\",\"${IRSA_ARN//\"/\"\"}\""
