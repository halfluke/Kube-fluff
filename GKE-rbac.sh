...
#   2  → Optional GCP cluster IAM (high-privilege bindings on the cluster resource)
...
#   (region or zone) to list roles/container.clusterAdmin / container.admin on the cluster.
...
Description:
  ...
  Check 2 is optional: it lists high-privilege GCP IAM on the cluster when
  gcloud and GKE_PROJECT, GKE_CLUSTER_NAME, GKE_LOCATION are set (see output
  when check 2 runs without them). Check 22 flags WI-annotated SAs that also
...
Available checks (use --checks to select, comma-separated, ranges allowed):
  ...
  2  : Optional GCP cluster IAM — roles/container.clusterAdmin & container.admin (needs gcloud + env)
...
# Check 2: Optional audit of GCP IAM on the GKE cluster resource (cloud-side analogue to EKS aws-auth).
...
if should_run 22; then
  ...
  while IFS= read -r n;  do [[ -n "$n"  ]] && C22_CR_PRIV["$n"]=1; done <<<"$PRIV_CR_C22"
  while IFS= read -r n;  do [[ -n "$n"  ]] && C22_CR_SEC["$n"]=1;  done <<<"$SEC_CR_C22"
  while IFS= read -r n;  do [[ -n "$n"  ]] && C22_CR_POD["$n"]=1;  done <<<"$POD_CR_C22"
  while IFS= read -r n;  do [[ -n "$n"  ]] && C22_CR_TOK["$n"]=1;  done <<<"$TOK_CR_C22"
  while IFS= read -r nk; do [[ -n "$nk" ]] && C22_R_PRIV["$nk"]=1; done <<<"$PRIV_R_C22"
  while IFS= read -r nk; do [[ -n "$nk" ]] && C22_R_SEC["$nk"]=1;  done <<<"$SEC_R_C22"
  while IFS= read -r nk; do [[ -n "$nk" ]] && C22_R_POD["$nk"]=1;  done <<<"$POD_R_C22"
  while IFS= read -r nk; do [[ -n "$nk" ]] && C22_R_TOK["$nk"]=1;  done <<<"$TOK_R_C22"
